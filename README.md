...
El programa dice si la expresión es aceptada y muestra el árbol, o si es rechazada y en qué columna está el error. Le agregamos la resta (`E -> E - T`) porque los ejemplos de la diapositiva la usan aunque la regla no la tenga.

En las pruebas revisamos los tres ejemplos de la diapositiva y varias expresiones inválidas, como `2 + * 3`, `(2 + 3` o `2 (3 * 4)`. También revisamos que la multiplicación tenga más precedencia que la suma y que la suma y la resta agrupen por la izquierda.

## Punto 2: formas de AST

Usamos la misma gramática, pero con un **Visitor** (`ConstructorAST.java`) que convierte el parse tree en un AST. Para cada expresión se imprimen los dos árboles dibujados, cuántos nodos tiene cada uno y el AST en una línea, por ejemplo `(+ 3 (* 4 5))`.
...
**ANTLR no avisa que la gramática es ambigua**. La acepta sin problema y siempre arma un solo árbol. Lo que hace es darle más precedencia a la alternativa que está escrita primero. Por eso lo probamos de tres formas:
